                        ...
                        <div class="col-12 mt-3">
                            <div class="form-group mb-0">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <label class="title-color mb-0">{{ translate('quantity_tier_discounts') }}</label>
                                    <button type="button" class="btn btn-outline-primary btn-sm" id="vendor-tier-discount-add-row">
                                        {{ translate('add_tier') }}
                                    </button>
                                </div>
                                <div class="table-responsive">
                                    <table class="table table-bordered mb-0" id="vendor-tier-discount-table">
                                        <thead>
                                        <tr>
                                        ...
                                        <tbody>
                                        @php($tierMin = old('tier_min_qty', $product->tierDiscounts->pluck('min_qty')->toArray()))
                                        @php($tierMax = old('tier_max_qty', $product->tierDiscounts->pluck('max_qty')->toArray()))
                                        @php($tierType = old('tier_discount_type', $product->tierDiscounts->pluck('discount_type')->toArray()))
                                        @php($tierDiscount = old('tier_discount', $product->tierDiscounts->map(fn($item) => $item->discount_type == 'flat' ? usdToDefaultCurrency($item->discount) : $item->discount)->toArray()))
                                        @forelse($tierMin as $index => $minQty)
                                            <tr class="tier-discount-row">
                                                ...
                                                <td class="text-center">
                                                    <button type="button" class="btn btn-outline-danger btn-sm vendor-tier-discount-remove-row">{{ translate('remove') }}</button>
                                                </td>
                                            </tr>
                                        @empty
                                        @endforelse
