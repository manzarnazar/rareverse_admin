            ...
            <div class="card-body">
                <div class="bg-section rounded-10 p-12 p-sm-20">
                    <div class="row g-2 align-items-end">
                        ...
                        @php
                            $wholesaleTiers = old('wholesale_tiers', $product->wholesale_tiers ?? []);
                            if (is_string($wholesaleTiers)) {
                                $decodedTiers = json_decode($wholesaleTiers, true);
                                $wholesaleTiers = is_array($decodedTiers) ? $decodedTiers : [];
                            }
                            $wholesaleTiers = is_array($wholesaleTiers) ? $wholesaleTiers : [];
                        @endphp
                        <div class="col-12">
                            <div class="form-group mb-0">
                                <label class="title-color">{{ translate('Wholesale_Tiers') }}</label>
                                <p class="fs-12 text-muted mb-2">{{ translate('set_quantity_based_unit_price_for_bulk_orders') }}</p>
                                <input type="hidden" name="wholesale_tiers" id="vendor-update-wholesale-tiers-input" value='@json($wholesaleTiers)'>
                                <div class="table-responsive border rounded">
                                    <table class="table table-sm mb-0" id="vendor-update-wholesale-tier-table">
                                        <thead>
                                        <tr>
                                            <th>{{ translate('Min_Qty') }}</th>
                                            <th>{{ translate('Max_Qty') }}</th>
                                            <th>{{ translate('Variant_(optional)') }}</th>
                                            <th>{{ translate('Unit_Price') }}</th>
                                            <th class="text-end">{{ translate('Action') }}</th>
                                        </tr>
                                        </thead>
                                        <tbody id="vendor-update-wholesale-tier-rows"></tbody>
                                    </table>
                                </div>
                                <button type="button" class="btn btn-outline-primary btn-sm mt-2" id="vendor-update-add-wholesale-tier-row">
                                    {{ translate('Add_Tier') }}
                                </button>
                            </div>
                        </div>
...
<script>
    (function () {
        const rowsContainer = document.getElementById('vendor-update-wholesale-tier-rows');
        const addRowButton = document.getElementById('vendor-update-add-wholesale-tier-row');
        const tiersInput = document.getElementById('vendor-update-wholesale-tiers-input');
        if (!rowsContainer || !addRowButton || !tiersInput) {
            return;
        }

        const parseInitial = () => {
            try {
                const parsed = JSON.parse(tiersInput.value || '[]');
        ...
        const serializeRows = () => {
            ...
            const tiers = rows.map((row) => {
                const minQty = row.querySelector('.wh-tier-min')?.value?.trim();
                const maxQty = row.querySelector('.wh-tier-max')?.value?.trim();
                const variant = row.querySelector('.wh-tier-variant')?.value?.trim();
                const unitPrice = row.querySelector('.wh-tier-price')?.value?.trim();
                if (!minQty || !unitPrice) {
                    return null;
                }
                return {
        ...
        const attachListeners = (row) => {
            ...
            row.querySelector('.wh-tier-remove')?.addEventListener('click', function () {
                row.remove();
                serializeRows();
            });
        };
        ...
        const addRow = (tier = {}) => {
            ...
            row.innerHTML = `
                <td><input type="number" min="1" class="form-control form-control-sm wh-tier-min" value="${tier.min_qty ?? ''}"></td>
                <td><input type="number" min="1" class="form-control form-control-sm wh-tier-max" value="${tier.max_qty ?? ''}"></td>
                <td><input type="text" class="form-control form-control-sm wh-tier-variant" value="${tier.variant ?? ''}" placeholder="Color-Size"></td>
                <td><input type="number" min="0" step="0.01" class="form-control form-control-sm wh-tier-price" value="${tier.unit_price ?? ''}"></td>
                <td class="text-end"><button type="button" class="btn btn-outline-danger btn-sm wh-tier-remove">X</button></td>
            `;
            rowsContainer.appendChild(row);
            attachListeners(row);
        };

        const initialTiers = parseInitial();
        if (initialTiers.length > 0) {
            initialTiers.forEach((tier) => addRow(tier));
        } else {
            addRow();
        }
